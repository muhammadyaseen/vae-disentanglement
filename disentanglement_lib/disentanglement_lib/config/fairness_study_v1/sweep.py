...
"""Hyperparameter sweeps and configs for the study "fairness_study_v1".
...
class FairnessStudyV1(study.Study):
  ...
  def get_model_config(self, model_num=0):
    ...
    model_config_file = resources.get_file(
        "config/fairness_study_v1/model_configs/shared.gin")
  ...
  def get_postprocess_config_files(self):
    ...
    return list(
        resources.get_files_in_folder(
            "config/fairness_study_v1/postprocess_configs/"))

  def get_eval_config_files(self):
    """Returns evaluation config files."""
    return list(
        resources.get_files_in_folder(
            "config/fairness_study_v1/metric_configs/"))

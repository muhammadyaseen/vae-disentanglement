...
class LadderVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        # bottom-up path
        self.nn_d_1 = encoders.SimpleConv64(latent_dim=self.z_dim * 2, num_channels=self.num_channels, image_size=self.image_size)
        self.nn_d_2 = encoders.SimpleFCNNEncoder(latent_dim=self.z_dim * 2, in_dim=self.z_dim * 2, h_dims=[self.z_dim * 2])       
     
        # top-down path
        self.nn_z_1 = decoders.SimpleFCNNDecoder(latent_dim=self.z_dim * 2, in_dim=self.z_dim, h_dims=[self.z_dim * 2])
        self.nn_x = decoders.SimpleConv64(latent_dim=self.z_dim, num_channels=self.num_channels, image_size=self.image_size)
    ...
    def encode(self, x_true, **kwargs):
        ...
        # step 1
        r_1 = self.nn_d_1(x_true) # use encoder for this nn
        
        #step 2
        mu_q_1_hat, log_var_q_1_hat = torch.chunk(r_1, 2, dim=1)
        #delta_log_var_1 = F.hardtanh(delta_log_var_1, -7., 2.)
        
        # step 3
        r_2  = self.nn_d_2(r_1)
        mu_q_2_hat, log_var_q_2_hat = torch.chunk(r_2, 2, dim=1)
        ...
        # mu_q_1, log_var_q_1 i.e. var dist params for q(z_1|z_2,x) depend on params computed at bottom up level
        # so we compute those here
        prec_q_1_hat = log_var_q_1_hat.exp().pow(-1)
        prec_p_1 = log_var_p_1.exp().pow(-1)

        mu_q_1 = (mu_q_1_hat * prec_q_1_hat + mu_p_1 * prec_p_1) / ( prec_q_1_hat + prec_p_1)
    ...
    def sample(self, num_samples, current_device):
        ...
        # step 1
        z_2 = torch.randn(num_samples, self.z_dim)
